...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    return {k: meter.global_avg for k, meter in metric_logger.meters.items()} #, listR


@torch.no_grad()
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, output_dir, fp16=False):
    model.eval()
    criterion.eval()
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        for key, v in res.items():
            for s,l,k in zip(v['scores'], v['labels'], v['keypoints']):
